...
export class MerchantController {
  ...
  async getConnectors(
    ...
    try {
      let merchants = [];
      const isPlatformOrg = organizationId === "platform-org-id";

      if (includeInactive === 'true') {
        const { PrismaClient } = require('@prisma/client');
        const prisma = new PrismaClient();
        if (organizationId && !isPlatformOrg) {
          merchants = await prisma.merchant.findMany({ where: { organizationId, isPlatform: false, deletedAt: null }, include: { providers: true } });
        } else if (isPlatformOrg) {
          merchants = await prisma.merchant.findMany({ where: { isPlatform: false, deletedAt: null }, include: { providers: true } });
        }
      } else {
        if (organizationId && !isPlatformOrg) {
  ...
  async deleteMerchant(
    ...
    const result = await this.merchantService.deleteMerchant(merchantId, organizationId, userType);
    if (userId) {
      await logAuditActivity(
        "MERCHANT_DELETED",
        merchantId,
